...
def test_full_pay_flow(get_token):
    ...
    #数据库查账：统一使用 db 工具，避免在用例中硬编码连接信息
    # 1. 查订单表：看状态是不是变成了 1（已支付）
    order_data = db.query_one(f"SELECT status FROM orders WHERE id = {order_id}")
    print(f"[核查] 订单当前状态为: {order_data['status']}")
    assert order_data['status'] == 1  # 极其严格的断言！

    # 2. 查用户表：刚才扣了 100 块，看 kobe 的余额是不是 400.00 了！
    user_data = db.query_one("SELECT balance FROM users WHERE username = 'kobe'")
